\
// robust25.7-minigui: pannello unico "Now Playing" + bottoni; i comandi testuali vengono auto-cancellati
...
const PREFIX = '!'
const DELETE_COMMANDS = (process.env.DELETE_COMMANDS || 'true').toLowerCase() === 'true' // cancella i messaggi con comandi
const SHOW_HELP_ON_START = false

const queues = new Map()

// tema minimal
const THEME = { color: Number(process.env.THEME_COLOR || 0xED4245) }
const EMO = { prev:'⏮️', pause:'⏸️', play:'▶️', stop:'⏹️', skip:'⏭️' }
...
function ensureGuild(guildId, channel){
  if(!queues.has(guildId)){
    ...
    const data = { queue: [], player, textChannel: channel, current: null, playing: false,
      nowTitle: null, controlsMsgId: null, thumbnail: null, uploader: null, duration: null, requesterId: null, requesterTag: null }
    player.on(AudioPlayerStatus.Idle, () => { data.playing = false; cleanup(data); upsertPanel(channel.guild.id, 'idle'); if (data.queue.length) playNext(channel.guild.id).catch(()=>{}) })
    player.on('error', e => { console.error('[player]', e); data.playing = false; cleanup(data); upsertPanel(channel.guild.id, 'idle'); if (data.queue.length) playNext(channel.guild.id).catch(()=>{}) })
...
function resolveMeta(input){
  return new Promise((resolve) => {
    ...
    p.on('close', () => {
      ...
      resolve({ title: title||input, uploader: uploader||'YouTube' })
    })
  })
}
...
// ---- Pannello Now Playing (minimal, senza flood) ----
function panelEmbed(data, status='idle'){
...
function panelButtons(status='idle'){
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('ctrl_prev').setEmoji(EMO.prev).setStyle(ButtonStyle.Secondary).setDisabled(true), // non implementato (no "previous" buffer)
...
async function upsertPanel(guildId, status='idle'){
  const data = queues.get(guildId); if (!data || !data.textChannel) return
  const embed = panelEmbed(data, status); const comps = panelButtons(status)
  try{
...
async function playNext(guildId){
  ...
  try {
    ...
    setTimeout(()=> upsertPanel(guildId, data.player.state.status===AudioPlayerStatus.Playing?'playing':'idle'), 1500)
  } catch (e) {
